		...
		float yOffset)
{
	...
	HR(D3DXCreateTextureFromFile(gd3dDevice, (LPCWSTR)tex0.c_str(), &mTex0));
	HR(D3DXCreateTextureFromFile(gd3dDevice, (LPCWSTR)tex1.c_str(), &mTex1));
	HR(D3DXCreateTextureFromFile(gd3dDevice, (LPCWSTR)tex2.c_str(), &mTex2));
	HR(D3DXCreateTextureFromFile(gd3dDevice, (LPCWSTR)blendMap.c_str(), &mBlendMap));
...
void Terrain::buildEffect()
{
	...
	HR(D3DXCreateEffectFromFile(gd3dDevice, (LPCWSTR)"Terrain.fx",
		0, 0, D3DXSHADER_DEBUG, 0, &mFX, &errors));
	if( errors )
		MessageBox(0, (LPCWSTR)(char*)errors->GetBufferPointer(), 0, 0);
